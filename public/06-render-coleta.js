...
// Cada um vai numa CAIXA de tamanho fixo (mesma largura/altura), centralizado — assim
// logos largos (Amazon/Magalu) e quadrados (ML/Shopee) ficam visualmente uniformes.
function bolinhaMkt(mkt){
  var m=MKT[mkt]; if(!m) return '';
  var caixa='display:inline-flex;align-items:center;justify-content:center;width:48px;height:22px;margin-right:8px;vertical-align:middle;flex:none';
  if(MKT_LOGOS[mkt]){
    return '<span title="'+m.n+'" style="'+caixa+'"><img src="'+MKT_LOGOS[mkt]+'" alt="'+m.n+'" style="max-width:48px;max-height:22px;object-fit:contain;border-radius:3px"></span>';
  }
  var cor=m.dot||'#8E8E93';
  var borda=m.ring?'border:2px solid #fff;':'border:1px solid rgba(255,255,255,.22);';
  return '<span title="'+m.n+'" style="'+caixa+'"><span style="display:inline-block;width:14px;height:14px;border-radius:50%;background:'+cor+';'+borda+'box-sizing:border-box"></span></span>';
}
